...
pub fn interpret(bytecodes: Vec<ByteCode>) -> Result<Variable> {
    ...
    for op in code.bytecodes {
        if let Some(err) = match op {
            ...
            ByteCode::Mul => execute_operation!(code, *),
			ByteCode::Div => execute_operation!(code, /),
            ByteCode::Add => execute_operation!(code, +),
			ByteCode::Sub => execute_operation!(code, -),
